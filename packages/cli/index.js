...
inquirer
  .prompt([
    ...
    { type: "input", name: "url", message: "What is the article url?" },
    {
      type: "list",
      name: "articleType",
      message: "What type of article?",
      choices: ["algorithm", "leetcode"],
    },
    {
      type: "confirm",
      name: "addSnippet",
      message: "Add a code snippet?",
      default: false,
    },
  ])
  .then(({ addSnippet, articleName, url, articleType }) => {
    const formatUrl = url
      ...
      .filter((s) => s)
      .join("/");
    const dirName = articleType === "leetcode" ? `leetCode/${formatUrl}` : url;
    const snippetLocation = addSnippet
      ? path.resolve(...blogLocation, "codesnippets/", `${dirName}`)
      : false;
    ...
    if (snippetLocation) {
      ...
      try {
        fs.writeFileSync(
          `${snippetLocation}/index.js`,
          snippetTemplate(),
          "utf-8"
        );
        fs.writeFileSync(
          `${snippetLocation}/implementation.js`,
          snippetTemplate(),
          "utf-8"
        );
      } catch (err) {
    ...
    const defaultTempate = template({
      date: new Date().toISOString(),
      title: articleName,
      snippet: snippetLocation ? `@codeSnippets/${dirName}` : undefined,
    });
